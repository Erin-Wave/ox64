...
- **체결가 = 봇("AI") 이 만든 합성 시세**: `functions/api/spot.ts` 의 예약된 봇 유저 2명(`bot-mm-1`/
  ...
  결정론적 알고리즘.
- **⚠⚠⚠ 관심도(interest) · business time · 기준선 tether 제거 (2026-09-23) — 아래 옛 설명들보다 이 절이 우선한다**:
  제보 둘 — ① "24시간 너무 활발하다(관심도가 떨어지면 체결도 안 되고 호가도 덜 바뀌고, 거래량이 터지면 급변하게)"
  ② "1 USDT 위면 무조건 하락, 아래면 무조건 상승". prod 실측도 그랬다: EW 3일 새 35→302→3.2→20.6, OX 1.4→7.3,
  **시간봉이 매시간 30~100%**. 원인은 셋이었다.
  - **(1) 매 틱이 똑같이 붐볐다** — 체결은 틱당 최소 2건 보장, 호가 교체·사건 확률은 "틱당" 상수라 새벽이든 저녁이든
    같은 밀도였다(세션 배수 0.7~1.4 뿐).
  - **(2) 틱 간격이 곧 시장 속도였다** — 모든 상수가 틱당이라 **보고 있을 때(폴링 초당 1틱)가 안 볼 때(cron 분당 12틱)보다
    5배 빨리** 돌았다. sim 은 분당 20틱으로 튜닝돼 있었으니 유저 화면은 튜닝값보다 3배 빨랐다(1분봉 2.9% → 실제 ~5%).
  - **(3) 기준선 tether 가 "돌아가야 할 가격"을 만들었다** — 적정가를 1 로 당기는 힘이 로그 거리의 제곱이라 EW 20.6 에서
    적정가 하락 압력이 폴링 기준 분당 1.9%(상한 1%/틱에 고착). 게다가 **tether 를 끄고 재니 국면 bias 자체가 +27e-6/틱의
    상승 편향**이었다 — "점유율 가중 합이 0" 은 tether 가 가려준 착시였고, tether 가 그 편향을 1 위에선 끌어내리고
    1 아래선 밀어올리며 눌러온 것이 곧 제보 ② 의 정체다. 예전 모델(tether 포함)을 시작가만 바꿔 하루 돌리면
    **0.2 → +409%, 1 → +95%, 5 → −59%, 20 → −88%** 다.
  고친 것(`functions/api/spot.ts`, 전부 순수 계산 — **D1 쿼리·쓰기 행 수 증가 0**, 상태 행 컬럼만 셋 추가):
  - **관심도 I = 시장 시간이 흐르는 속도(subordination)**. 가격·국면·심리·호가 교체는 전부 business time
    `h = (이 틱이 덮는 벽시계 초 / TICK_REF_SEC=3) × I` 로 진행한다. I 가 낮으면 한 틱에 체결 0~1건·호가 거의 정지·가격은
    한두 틱 사이, 몰리면 같은 1초에 몇 틱치 시장이 흐른다(체결 폭주·호가 급변·큰 봉). 거래량과 변동성이 같이 뭉치는
    실제 시장의 이유가 이것이다.
    ⚠ **핵심 불변식: business time 동역학은 예전 모델 그대로다**(진폭만 `MOVE_SCALE` 배). 그래서 이미 튜닝된 성질
    (추세효율·국면 점유율·되돌림·공포 게이지·사건 빈도)이 보존되고, 관심도가 국면마다 달라도(패닉엔 heat↑) 장기
    편향이 안 생긴다 — 가격이 한 단위 움직일 때의 확률 법칙이 관심도와 무관하기 때문이다. **새 항을 추가할 땐 이
    규칙을 지킬 것**: AR 지속계수는 `계수^h`, 사건 확률은 `1 − e^(−율×h)`, 이동은 `× h`, 노이즈는 `× √h`
    (h=1 이면 예전 식과 정확히 같다). 틱당 상수를 그대로 쓰면 틱 간격에 따라 시장이 달라진다.
  - **관심도는 벽시계로 흐른다**(사람이 식는 속도): 목표 = `INTEREST_BASE(0.14) × 세션^1.4 × 화제성(hype) × 국면 heat ×
    (1 + 0.8·공포 + 0.45·탐욕)`, 로그 공간 OU 로 목표에 되돌아간다(τ 300초, 요동 sd 0.45). 오래 관망하면 heat 가 더
    떨어진다(`CALM_HEAT_FLOOR`). 화제성은 6시간짜리 느린 성분(어떤 날은 종일 붐빈다).
    · **뉴스** — 벽시계 Poisson(시간당 0.3 × 세션 × √화제성), 관심도 ×2~×15(지수 꼬리). 60% 는 방향(호재/악재 **50:50**)을
      drift·무드에 싣는다 — 거래량이 "갑자기" 터지는 사건의 원천.
    · **자기 흥분(buzz)** — 돌파 추격·손절 연쇄·급등락 점프·팻테일·패닉/광기 진입이 관심을 끈다.
      ⚠⚠ **포화 필수**(`BUZZ_SAT`): 사건은 business time 으로 일어나므로(관심도↑ → 초당 사건↑) 그대로 두면
      "관심 → 사건 → 관심" 폭주다. 첫 구현에서 관심도가 상한(10)에 한 주 내내 붙었다(Hawkes 분기비 > 1).
      `buzz × SAT/(SAT+I)` 로 이미 관심이 높을수록 사건 하나의 효과를 줄였다.
      ⚠ 같은 이유로 **뉴스 도착률은 화제성에 √ 로만** 걸고, 사건은 화제성에 아무것도 남기지 않는다(첫 구현: 도착률 ∝
      화제성 → 뉴스 하루 53회, 화제성 상한 고착).
    · **유저 대량 체결** — 시장가·청산·지정가 체결·SL/TP 가 쓰는 기준가 upsert 에 `INTEREST_BUMP_SQL` 을 붙여 관심도에
      `1 + 0.14·√(수량/2370)` 을 곱한다(5천 개 ×1.2 · 10만 개 ×2 · 100만 개 ×4). 같은 문장에 컬럼 하나 — 행·문장 증가 0.
  - **틱 간격을 흡수한다(`dtSec`)** — `runBotTicks` 가 `last_run` 에서 경과 시간을 재 틱 수로 나눠 넘긴다(폴링 ~1초 1틱,
    cron ~60초를 12틱). 상한 `ELAPSED_MAX_MS`(2분)·`DT_MAX_SEC`(15초). 폴링이 오래 쉬었다 돌면 3초 틱 여러 개로 나눠
    따라잡는다(`POLL_CATCHUP_MAX_TICKS`=12, 틱은 메모리 → 커밋 1회라 D1 무관). 틱 시각(ts)은 여전히 10ms 씩 앞당겨
    찍는다(과거 소급 금지 — 마감된 봉 변조).
  - **체결 = Poisson(λ), λ = 10 × h × √sizeMult**(0 건 허용). 상한 40 에 잘리면 그만큼 한 건을 키운다(cron 과열 틱의
    거래량 보존). 평균 체결 크기는 관심도^0.2 로 완만히 커진다(사람이 몰리면 큰손도 온다).
  - **호가 교체도 business time** — 취소·수량변경 확률을 `1 − (1 − p)^bookPace` 로(bookPace = h, 하한 `BOOK_PACE_FLOOR`
    0.3 상당). 새로 까는 호가 두께 × `I^0.3`(0.5~1.7), 한산하면 스프레드 ×(1 + 0.3·(1−I)), 유저 벽을 갉는 양 × h.
  - **⚠⚠ 마지막 체결을 기준가에 강제하지 않는다** — 예전엔 틱의 마지막 체결가 = ref 였는데, 한 틱 체결이 0~1건이 되자
    **한산한 시장의 체결이 전부 호가 한가운데(mid)** 에 찍혔다(실거래소에 없는 가격, 호가 바운스 소멸). 지금은 모든 체결이
    bid/ask 에 찍히고 ref 는 **공정가(mid)** 다. 캔들 종가(마지막 체결가)와 헤더 현재가(mark)가 반 스프레드쯤 다를 수 있다
    (실거래소 last/mark 와 같다). ⚠ 그래서 **헤더 현재가는 서버 mark 로만** 넣는다 — `?tick=` 응답의 `mark`(TickCtx.mark)를
    `useTradingStore.spotTick` 이 `setPrice`, 차트(Chart.tsx)는 가상 코인에서 setPrice 를 안 한다(둘 다 넣으면 1초/3초
    주기로 번갈아 깜빡인다). `TickCtx.mark` 는 `ref` 와 달리 sweep 이 대기 지정가를 건드려도 null 이 안 된다 —
    `ref` 는 트리거 시드라 대기 주문이 하나라도 있으면(체결이 안 나도) null 이 된다(`sweepRestingOxPendings` 가
    시도만 해도 touched).
  - **공정가는 틱에 스냅하지 않는다**(`BotState.ref` 원본 → `fair` 컬럼, 반올림값 → `ref_price`). 한산한 틱은 한 틱
    (가격 1.x 에선 0.1%!)보다 작게 움직이는데 매번 반올림하면 이동이 전부 버려져 가격이 얼어붙는다. ⚠ `fair` 는
    **`roundOx(fair) === ref_price` 일 때만** 이어 쓴다(`toBotState`) — 유저 체결은 ref_price 만 바꾸므로 어긋났다면 유저가
    가격을 옮긴 것이고, 그땐 ref_price 에서 다시 시작해야 시장충격이 안 지워진다(동기화 코드 없이 자기 검증).
  - **tether 제거 + 편향을 직접 0 으로** — `BOT_BASE_PULL`/`BOT_BASE_PRICE` 삭제. 상승 국면 bias 를 4.6% 낮추고
    (rally 0.00106→0.00101, euphoria 0.0034→0.00324) 남은 잔차를 `DRIFT_TRIM`(20.8e-6 × MOVE_SCALE / 단위)으로 뺐다.
    ⚠ **되돌려줄 힘이 없으므로 남은 편향은 그대로 추세다**(단위당 1e-6 ≈ 하루 1%) — 국면·심리·관심도를 건드렸으면
    반드시 다시 잴 것(아래 검증 방법). 남은 되돌림은 전부 **움직이는 적정가**(anchor, 반감기 69단위) 기준이다.
  - **진폭 `MOVE_SCALE`=0.2** — 가격 단위를 가진 상수(bias·노이즈·킥·마찰·과열 문턱·게이지·영향권·점프)에 곱한다.
    가격 거리에 곱하는 심리 계수(45×drift, 1.6×stretch)는 ÷, 3차 되돌림 계수는 ÷². 무차원 상수(지속계수·전이율·수명)는
    그대로라 **모양은 같고 크기만 준다**. 스톱헌팅 크기는 × √MOVE_SCALE.
  - **사건(돌파 추격·손절 연쇄)은 "이번 틱에 전 고점/저점을 넘어선 거리"에 비례한 확률로 발동**(`FOMO_RATE`=15,
    `CASCADE_RATE`=22, 킥은 다음 틱부터 drift 에). ⚠⚠ 예전의 "지금 정확히 고점에 있나"(ref ≥ peak) 한 점 조건은
    **틱이 굵을수록 참일 확률이 커서** cron(5초 틱)이 폴링(1초 틱)보다 사건을 더 만들었고, 연쇄가 더 크니 **안 볼 때만
    하락 편향**(하루 −1.7%)이 생겼다(sim 대조: 사건을 끄면 두 틱 간격의 편향이 같아진다). 돌파 거리의 누적(고점 기억의
    상승분)은 연속 시간에서 잘 정의되는 양이라 틱 간격과 무관하다.
  - **국면 전이는 경쟁 hazard**(`nextRegime`) — 분기마다 "단위당 발생률", 무슨 일이든 일어날 확률 `1 − e^(−Σ율×h)`,
    어느 쪽인지는 율 비례. 예전 "틱당 확률" 을 h 배 하면 큰 h 에서 1 에 잘려 **수명이 긴 광기가 짧은 패닉보다 덜 끊겨
    상승 편향**이 생긴다.
  - 검증(`npm run sim:bot` 기본 1초 틱 7일×8회 + 전용 하네스 192회):
    | 지표 | 개편 전(분당 20틱 sim) | 개편 후 |
    | --- | --- | --- |
    | 관심도 중앙 / 평균 / p90 | — | 0.18 / 0.33 / 0.71 (한산<0.25 시간 63%, 과열>2 1.5%, 뉴스 7.4회/일) |
    | 1분봉 폭 | 2.9% (폴링이면 ~5%) | 평균 0.19% · 한산 0.06% · 과열 1.73% |
    | 시간봉 폭 중앙(p90) / 일간 범위 | — (prod 매시간 30~100%) | 3.7%(8.1%) / 25% |
    | 스윙 | 8%+ 146회/일(평균 23%) | 5%+ 12.6회/일(평균 9%) |
    | 분당 거래량 p90/p10 · \|1분수익률\|↔거래량 상관 | — | 150배 · 0.57 |
    | 1초 틱 체결 없는 초 / 초당 체결 / 호가 지속 | 0% / ~12건 / 27% | 한산 74% / 0.34건 / 76% · 보통 30% / 2.1건 / 66% · 과열 1% / 14.8건 / 29% |
    | 체결 기준 1분봉 중앙(한산 / 보통 / 과열) · 최우선 스프레드 | — · 0.29% | 0.20% / 0.45% / 1.5~2.8% · 0.17~0.24% (아래 미세구조 항목 이후) |
    | 시작가 0.2 / 1 / 5 → 하루 뒤 | +409% / +95% / −59% | +2.8% / +3.0% / −1.1% (±1~2%, 수준 무관) |
    | 로그드리프트/단위(192회) | tether 끄면 +27e-6 | 1초 틱 +0.44 ± 0.36e-6 · 5초 틱 −0.64 ± 0.37e-6 |
    | 사건 1000단위당 추격 / 연쇄 | 5.5 / 6.8 | 1초 틱 5.49 / 7.01 · 5초 틱 5.35 / 6.43 |
    | business time: 추세효율 · 국면 수명 · 되돌림 몫 · 공포 | 0.61 · 69 · 38% · 0.29 | 0.62 · 71 · 40% · 0.27 (점유율 calm 38 / rally 24 / pullback 23 / panic 8.5 / euphoria 5.3 / cap 0.3) |
    | 틱 간격 불변(1초 vs 5초 틱) | 5배 차이 | 관심도 중앙 0.18/0.19 · 1분봉 0.19/0.20% · 시간봉 3.7/4.0% · 일간 범위 25/24% |
    | CPU(cron 24틱) | ~3.5ms | 1.0~1.6ms |
    ⚠ sim 의 business time acf1(~0.7)은 예전(0.55)보다 높게 나온다 — 과열 구간에서 한 틱이 여러 단위를 덮어 틱 안을
    선형 보간하기 때문이다(측정 artefact). 모양 비교는 추세효율·분산비·국면 수명으로 할 것.
    ⚠ 편향 판정은 sim 기본 8회로 하지 말 것(SE ~1.8e-6) — scratch 하네스처럼 **실행당 추정치 192회**로 SE ~0.35e-6 까지
    내려야 판정이 된다. 1초 틱과 5초 틱을 **둘 다** 재야 한다(이산화 의존 편향은 한쪽만 재면 안 보인다).
  - **⚠⚠ 한산한 시장에선 봉이 "스프레드 폭"이 된다 — 미세구조도 진폭에 맞췄다**(배포 후 prod 관측으로 드러남):
    공정가는 1분에 0.06% 움직이는데 **체결로 만든 1분봉은 0.3~0.8%** 였다. 예전엔 움직임이 커서 가려졌던 셋 때문이다.
    ①최우선 스프레드가 0.28~0.31%(오프셋 `SPREAD_BASE` 0.06% 에 격자 스냅·스프레드 배수·한산 가산이 얹힘) — 한산해도
    매수·매도가 번갈아 찍히면 봉이 곧 스프레드 폭이다 → `SPREAD_BASE`=`MICRO_HALF_SPREAD` 0.06→0.035%, `QUIET_SPREAD`
    0.3→0.15(실측 스프레드 0.17~0.24%). ②체결의 5틱 격자 스냅이 굵은 틱 가격대(19.55 → 5틱 0.26%, 1.x → 0.5%)에서 체결을
    **호가 바깥까지** 끌어갔다 → 스냅 거리가 반 스프레드를 넘으면 그 격자는 포기. ③고래 체결의 파고들기(`MICRO_DIG`,
    최대 0.8%)가 예전 진폭 그대로 → × √MOVE_SCALE(**쓰는 자리에서** — 모듈 상수에 곱하면 아래에 정의된 MOVE_SCALE 을
    초기화 전에 참조한다). 결과: 체결 기준 1분봉 중앙값 한산 **0.20%** · 보통 0.42~0.49% · 과열 1.5~2.8%(가격 1.5 / 7.6 / 19.5
    모두 비슷), 상승틱 매수라벨 64~80%.
    ⚠⚠ **호가는 틱에도 바깥쪽으로 맞춘다**(`humanQuotePrice` 폴백: 매수 내림·매도 올림). 공정가를 반올림하지 않게 되고
    스프레드를 좁히자 가격 1.x(틱 0.1%)에서 새 매도호가가 가장 가까운 틱(= 공정가 아래)으로 반올림돼 살아남은 매수호가와
    겹쳤다(sim 하루 41회 역전). 바깥쪽 반올림이면 매수 < 공정가 < 매도가 구조적으로 보장된다(1.0 경계를 오가는 경우 포함 0회).
  - **배포 시 1회 초기화**(prod 상태는 예전 진폭 기준): ALTER 3줄(§ CLAUDE.md §5) 뒤 `UPDATE spot_bot_state SET
    drift = drift * 0.2, peak = ref_price, trough = ref_price, anchor = ref_price` — 안 하면 새 게이지(만점 4.4%)로 보는
    옛 고점(8.7% 위)이 **공포 100%** 로 읽혀 첫 몇 분이 패닉이 된다.
  ...
  **`spot_bot_state` 행에 얹혀 틱 사이에 지속**되므로 추가 DB 왕복이 없다(어차피 읽고 쓰던 행 — 컬럼이
  ...
  - **⚠⚠ 다이내믹 재설계(2026-08-26) — "사팔사팔 빠르게 하면서 느리게 상승/하락추세"**: 위 항목들이
    ...
    **두 개의 숫자**였다:
    ...
    - ⚠ **2026-09-23 삭제됨**(§ 관심도 절 — tether 가 국면의 상승 편향을 가리며 "1 위면 무조건 하락"을 만들었다). 이력: 장기 tether(`BOT_BASE_PULL`)를 **로그 거리의 제곱**으로 바꿨다 — 되돌림을 약하게 만든 만큼
  ...
  - **⚠⚠ 사람 심리 4차 확장 — 하루 리듬 · 수축 후 확장 · 저항/지지 · 체결 미세구조(2026-09-07)**:
    ...
    **전부 벽시계·기존 상태(regimeTicks/peak/trough)에서 파생 — 컬럼 추가 0 = D1 읽기·쓰기 증가 0.**
    - **하루 리듬(세션·주말, `sessionActivity`)** — 아시아(UTC 03)·유럽(UTC 09)·유럽·미국 겹침(UTC 14:30,
      ...
      곱한다(⚠ **2026-09-23 부터는 관심도 목표값(× 활성도^1.4)에만** — 관심도가 business time 속도라 변동성·
      거래량·전이·사건에 한꺼번에 실린다. 따로 곱하면 이중 계상). ⚠⚠ **방향(bias)은 절대 안 건드린다** — 시간대 편향은 하루 단위로 누적돼 장기 안정성을
    ...
    - **전 고점·전 저점의 마찰(저항/지지)** — `peak`/`trough` 는 지금까지 게이지와 사건 방아쇠로만 쓰였다.
      ...
      반드시 다시 확인할 것.
    - ⚠ **2026-09-23 대체됨** — 지금은 돌파 거리 비례 확률(§ 관심도 절 "사건"). 이력: **사건 발생률이 심리·시간의 함수** — FOMO/손절연쇄 확률에 `활성도 × (0.55 + 1.1×게이지)` 를 곱하고
  ...
  - **⚠ 장기 안정성 — 파라미터를 바꿨으면 `npm run sim:bot` 을 반드시 돌릴 것** ⚠ 합격선은 2026-09-23 에 바뀌었다(§ 관심도 절 검증 표, scripts/sim-bot.ts 머리 주석이 진실원본) — 아래는 tether 가 있던 시절의 기록이다.(`scripts/sim-bot.ts`,
  ...
  - **⚠⚠ 기준가 클램프는 "시세 범위"가 아니라 0·Infinity 안전장치다(`_shared.VIRTUAL_PRICE_MIN/MAX`,
    ...
    **0.02**(2026-07-24 수정), 그다음이 **0.0001** 이었다. 둘 다 "그 시절의 최소 틱"을 하한에 그대로
    ...
    ⚠ (2026-09-23 이후 아래 문장은 무효 — tether 삭제, 가격은 로그 랜덤워크) **하한을 없앤다고 가격이 거기 머무르지는 않는다** — `BOT_BASE_PULL` 이 anchor 를 기준선 1 로 당기는
