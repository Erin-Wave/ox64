...
- 진폭은 `MOVE_SCALE`(0.2) 하나 — 가격 단위 상수엔 ×, 가격 거리에 곱하는 심리 계수엔 ÷, 3차 되돌림엔 ÷². 공정가(`fair`)는 틱에 스냅하지 않고 `ref_price` 에만 반올림값(유저 체결이 ref_price 를 바꾸면 `roundOx(fair)≠ref_price` 로 감지해 ref_price 에서 재시작). 틱의 마지막 체결을 ref 에 강제하지 않는다(모든 체결은 bid/ask).
