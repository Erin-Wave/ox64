...
├── schema.sql              D1 스키마(users/positions/orders/pending_orders) — wrangler d1 execute 또는 D1 Console 로 적용
...
│   ├── _trading.ts         checkTriggers(env,uid) — 지정가/SL/TP 체결 체크(서버는 cron 없음, 폴링 시점마다 평가)
│   └── api/
│       ├── login.ts        POST /api/login  (없는 이름=가입, 있으면 패스코드 검증→세션쿠키)
│       ├── logout.ts       POST /api/logout (쿠키 제거)
│       ├── state.ts        GET  /api/state  (checkTriggers 호출 후 잔고+포지션+주문+미체결주문, 인증필요)
...
└── src/                    ── 프론트 ──
    ...
    │   ├── useChartStore.ts    차트 옵션(indicators: 기간/개수 자유 설정 가능한 EMA/BB/RSI 배열, 카운트다운·거래량·매매마커·평단선) localStorage 영속
    ...
    │   └── useTradingStore.ts  서버 상태 캐시(positions/orders/pendingOrders) + init/login/logout/openMarket/closePosition/limitOpen/cancelLimit/setSlTp
    └── components/
        ├── Login.tsx           이름+패스코드 로그인/가입
        ├── Header.tsx          심볼(38, 공용목록)/현재가/연결/잔고/유저명/랭킹버튼/설정버튼/로그아웃
        ├── Settings.tsx        테마 3선택 + 거래모드(Easy/Standard) 2선택 모달
        ├── Chart.tsx           Lightweight Charts: 타임프레임 그룹셀렉트·KST+9·OHLCV레전드(hover/터치)·다음봉 카운트다운·인디케이터(추가/삭제/기간편집)·매매 B/S 마커·포지션 평단선·테마 반응형 캔버스 재도색
        ├── OrderPanel.tsx      Easy=시장가만 / Standard=시장가+지정가 탭·SL·TP 입력 (심볼/방향/수량/레버리지, 체결가는 서버가 fetch)
        ├── PositionsPanel.tsx  포지션 + 실시간 미실현 PnL/ROE%(전 심볼) + 청산 + (Standard) SL/TP 인라인 편집·미체결 지정가 목록
...
- **⚠ 체결 체크 = cron 없이 폴링 기반**: Cloudflare Pages Functions 는 정기 실행을 지원하지 않는다. 그래서 `functions/_trading.ts checkTriggers(env,uid)` 를 `state.ts`(GET, 클라가 `useTriggerPoll` 로 5초마다 호출)와 `order.ts`(POST 액션 진입 직후, 수동 조작과의 레이스 방지)에서 호출해 **그 유저의 요청이 들어올 때만** 지정가/SL/TP 를 평가·체결한다. 즉 아무도 앱을 켜 두지 않은 동안은 체결되지 않음(지인 대상 모의투자라 허용된 트레이드오프). 체결가는 지정가/SL/TP 값 그대로 사용(슬리피지 모델링 없음).
- **아직 없음**: 부분 청산, 펀딩비, 강제청산, 수수료.
...
- **⚠ SL/TP 마이그레이션(최초 1회, 수동)**: 지정가/SL/TP 기능 추가로 `positions` 에 `stop_loss`/`take_profit` 컬럼이 필요한데, prod D1 은 이미 스키마가 적용된 상태라 `CREATE TABLE IF NOT EXISTS` 로는 컬럼이 추가되지 않는다. `schema.sql` 맨 아래 주석 처리된 `ALTER TABLE positions ADD COLUMN stop_loss REAL;` / `... take_profit REAL;` 두 줄을 **한 번만** 직접 실행할 것: `npx wrangler d1 execute ox64 --remote --command "ALTER TABLE positions ADD COLUMN stop_loss REAL"` 및 `take_profit` 도 동일하게. 이미 실행했다면 재실행 시 "duplicate column name" 에러 발생(무시 가능, 이미 적용됐다는 뜻).
...
- [x] 사용자 설정 UI(테마 다크/라이트/고대비, 거래모드 Easy/Standard)
- [ ] 강제청산가 계산 + 표시
