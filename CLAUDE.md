...
- **⚠ 거래 수수료 + VIP 등급(2026-07-20)**: 모든 체결에 `수수료 = 명목금액(체결가×수량) × VIP 요율` 이 붙는다.
  - **등급 = 누적 거래대금(`users.total_volume`)** 으로 결정. 증거금이 아니라 **명목금액(레버리지 포함)** 이라 고배율일수록 빨리 오른다. 진입·청산 각각 그 체결의 명목금액만큼 누적. VIP0~12(2026-07-24 에 VIP4 가 너무 쉬워 5~12 프레스티지 등급 추가 — 2단계마다 ~10배).
    | 등급 | 누적 거래대금(USDT) | 요율 |
    | --- | --- | --- |
    | VIP0 | ~100만 | 0.03% |
    | VIP1 | 100만~1억 | 0.02% |
    ...
    | VIP5 | 3조~10조 | 0.0009% |
    | VIP6 | 10조~30조 | 0.0008% |
    | VIP7 | 30조~100조 | 0.0007% |
    | VIP8 | 100조~300조 | 0.0006% |
    | VIP9 | 300조~1000조 | 0.0005% |
    | VIP10 | 1000조~3000조 | 0.0004% |
    | VIP11 | 3000조~1경 | 0.0003% |
    | VIP12 | 1경~ | 0.0002% |
    - ⚠ VIP5+ 요율은 0.001% 미만이라 표시를 **`format.ts fmtFeeRate`(소수 4자리)** 로 통일했다(예전 `toFixed(3)` 은 0.001% 미만을 "0" 으로 뭉갰다). `VipBadge` 색 스케일도 13단계로 확장(VIP5~12 골드→그라디언트). `fmtKor` 에 경(1e16)/해(1e20) 단위 추가.
