...
└── src/                    ── 프론트 ──
    ...
    │   ├── indicatorDefs.ts 인디케이터 레지스트리(`INDICATOR_DEFS`): 타입별 라벨·패널(overlay=캔들 위 / own=하단 별도 패널)·파라미터 정의(key/label/기본값/범위)·선 스펙(kind line/hist/dots, 스타일, 고정색)·기준선(RSI 70/30 등)·국면 판정(`states` — 봉마다 라벨 인덱스, `colorByState` 선은 점마다 라벨 색 + 레전드에 라벨. OBV = 기준선(OBV 의 EMA, 점선) 대비 유입/유출 × 가격 EMA 대비 위/아래 → 매집·끌어올림·정리·하락·반등. ⚠ 두 판정 모두 **히스테리시스**(`band`, 기본 0.5 = 자기 평균 편차의 0.5배를 벗어나야 뒤집음, 기준선 위아래 점선이 그 경계 — `legend:false` 선) — 실측 3.6만 봉에서 전환 23.9→11.4/100봉·1~2봉 잔떨림 62→31%, 사후 정답 일치 60.0→59.2%. 일치율 천장(~75%/성분)은 추정식을 바꿔도 안 오른다(원리적 지연) — 밴드를 더 키우면 지연만 늘어난다. ⚠ "유출·가격 위"는 끌어올림을 거쳤을 때만 정리, 하락 뒤면 반등(경로 의존). ⚠ OBV 는 누적값이라 0 을 기준선으로 쓰지 말 것 — 과거봉 로드마다 통째로 이동한다. ⚠ LWC 는 선분을 시작점 색으로 그어서 `stateLine` 이 점 i 에 i+1 의 국면 색을 넣는다)·값 포맷·compute. **Chart 는 이 표만 보고 그리므로 지표 추가 = 여기 한 항목 + indicators.ts 계산 함수**(Chart/스토어에 타입 분기를 새로 넣지 말 것)
...
- **심리 파라미터를 바꿨으면 `npm run sim:bot`** — 합격선(1초 틱): 관심도 중앙 ~0.2·한산(<0.25) 절반 이상·과열(>2) 몇 %, 1분봉 한산 ~0.06%/과열 ~1.7%, 시간봉 중앙 ~3~5%, 일간 범위 ~20~30%, 체결 없는 초(한산) 70%+, 시작가 0.2/1/5 → 하루 뒤 셋 다 0 근처, business time 추세효율 ≥0.55·되돌려주는 몫 <55%·국면 수명 ~70·평균 공포 ~0.3·국면 점유율 calm 39/rally 24/pullback 23/panic 9/euphoria 5/capitulation 0.3%, 사건 1000단위당 추격 ~5.4/연쇄 ~6.7. `bias` 와 국면 수명은 같이 재조정. 되돌림을 풀면 `GAUGE_FULL` 을 같이 넓힐 것. `MOOD_PERSIST+HERD_GAIN<1` 유지.
- 크기 분포·호가 물량을 바꿨으면 sim 이 아니라 **상태 고정 A/B**(`simulateTick` 20만 회)로 "평균 보존"을 잰다(`SIZE_TIER_MEAN`/`FLOW_NORM`/`FLURRY_MEAN` 정규화). 쪼개기(`SLICE_CHANCE`)는 뽑힌 수량과 무관하게 결정.
