...
├── schema.sql              D1 스키마(users[+refill_count/refill_date]/positions/orders/pending_orders) — wrangler d1 execute 또는 D1 Console 로 적용
...
└── src/                    ── 프론트 ──
    ...
    └── components/
        ...
        ├── Header.tsx          심볼(38, 공용목록, 정렬 가능)/현재가/연결/잔고/리필버튼(N/3)/랭킹버튼/설정버튼/로그아웃
        ...
        ├── OrderPanel.tsx      Easy=시장가만 / Standard=시장가+지정가 탭·SL·TP 입력, 수량은 레버리지 스타일 슬라이더 + 단위(코인/USDT) 전환 (심볼/방향/수량/레버리지, 체결가는 서버가 fetch)
        ├── PositionsPanel.tsx  탭: 포지션(청산가 표시·부분청산 입력·(Standard) SL/TP 인라인 편집) / (Standard) 미체결 지정가 / 주문내역(전체 체결 이력, 강제청산 하이라이트)
...
- **리필(`functions/api/refill.ts`)**: 강제청산으로 잔고가 0이 됐을 때를 위한 안전망. `users.refill_count`/`refill_date`(KST 날짜)로 **1일 최대 3회, 1회 +10,000 USDT**. 날짜가 바뀌면 `refill_date !== 오늘` 이라 카운트를 0으로 취급(별도 리셋 cron 불필요 — `checkTriggers` 와 같은 "폴링 시점에 계산" 패턴). `loadState` 가 `refillsLeft` 를 계산해 응답에 포함.
