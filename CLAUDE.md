...
- **청산(close)**: **실제 코인 38종**은 서버가 청산가 fetch → `pnl` 계산 → 잔고에 `margin+pnl` 반환, 포지션 DELETE, close 주문 기록(pnl 포함). 전부 batch. `size` 를 지정하면 **부분 청산**(보유수량보다 작을 때) — 증거금/포지션 수량을 비율만큼만 줄이고 포지션은 유지, 생략/전량이면 DELETE(로컬 호가창이 없어 외부시세 mark 정산이 표준, 유동성 사실상 무한).
  ...
  - **⚠⚠ 유저 체결은 "한 줄"이 아니라 walking 한 만큼 여러 줄로 찍는다(`splitPrints`/`userTradeStmts`, 2026-09-03)**: 예전엔 시장가 한 방을 **1건으로 집계**해 찍어서 체결창에 "3,000,000개" 한 줄이 통째로 떴다("내 시장가가 합쳐서 나온다" 제보). 실제 거래소는 maker 주문 하나하나가 별도 체결로 인쇄되므로 사다리를 훑으면 그 단계 수만큼 줄이 생긴다. **집계했던 이유는 순전히 D1 비용**이다 — `spot_trades` INSERT 는 1건이 **3 rows written**(행 1 + 암묵 PK + 인덱스, §6)이라 실사다리 22단계 + 합성 24스텝을 전부 찍으면 체결 하나가 138행이 된다. 그렇다고 봇처럼 `tape_json` 링 버퍼에 얹으면 공짜지만 그 칸은 봇이 매 틱 통째로 덮어써서 **유저 체결이 몇 %는 사라진다**(자기 매매가 체결창에 안 보이는 건 "표시용 손실"로 넘길 수 없다). 그래서 **행으로 찍되 줄 수를 `USER_PRINT_MAX`(6)로 묶는다** — 같은 가격은 한 줄로 합치고(사다리 한 단계 = 한 체결), 그래도 넘치면 **연속 구간**을 가중평균가로 묶어 가격이 걸어간 순서를 유지한다. 작은 주문은 예전처럼 1~3줄, 아무리 큰 주문도 6줄(=18행)에서 멈춘다. 시각은 1ms 씩 벌려 찍는다(마지막이 정확히 `now`) — 같은 ms 에 몰면 정렬이 불안정하고, 클라가 새 체결을 **시각**으로 식별하므로(`dripTrades`) 한 건만 새 것으로 보고 나머지를 안 흘려보낸다. 적용 경로는 시장가 진입/청산·지정가 체결 셋이고, `recordVirtualFill`(SL/TP 등 mark 정산)은 walking 이 아니라 1건 그대로다. ⚠ 줄 상한을 올리면 §6 의 `ROWS_PER_FILL`(24)도 같이 올릴 것.
...
- **⚠ 봇 매매 심리 모델(`nextMarketState`, 2026-07-20)**: 예전 기준가는
  ...
  **`spot_bot_state` 행에 얹혀 틱 사이에 지속**되므로 추가 DB 왕복이 없다(어차피 읽고 쓰던 행 — 컬럼이
  ...
  - **⚠ 탐욕/공포 심화(2026-08-12)**: 위 항목들은 **가격의 통계적 성질**이지 사람의 행동이 아니었다 —
    ...
    **자기 고점/저점을 기억하지 않아** 저항 돌파·지지 붕괴 같은 사람이 읽는 사건이 아예 없었다. 그래서:
    ...
    - **고점/저점 기억(`peak`/`trough`)** — 새 극값이면 즉시 갱신, 아니면 현재가 쪽으로 서서히 잊힌다
      ...
      (`GAUGE_FULL`=14% 에서 100%)을 뽑아 전이 확률·변동성·거래량·호가 두께에 먹인다.
  ...
  - **⚠ 장기 안정성 — 파라미터를 바꿨으면 `npm run sim:bot` 을 반드시 돌릴 것**(`scripts/sim-bot.ts`,
    ...
    5~20일치에서 가격이 시작가의 0.5~2배 안, 수익률 lag1 자기상관 ~0.5(추세), |수익률| lag1 자기상관
    ~0.43(변동성 뭉침), 1분봉 평균 고저폭 ~2.7%, **추세효율 0.55 이상**(이게 낮으면 "사팔사팔"이 돌아온
    것이다), 국면 점유율이 위 표와 비슷할 것. 실측(45일×3회): 종가 0.71~1.92, 범위 0.16~5.35, MDD -96%.
  ...
  **⚠⚠ 2026-08-14 부터 이 사이트는 Workers *Free* 플랜을 목표로 운영한다.** 그래서 판단 기준이 완전히
  ...
  - **⚠⚠ 방어는 2겹이다 — Cloudflare 는 D1 에 지출 상한(hard cap) 기능을 제공하지 않는다.**
    ...
    2. **코드가 스스로 멈춘다 — `functions/_budget.ts` + `usage_meter` 테이블.**
       ...
       - ⚠ 체결 단가는 **20 → 24 로 올렸다(2026-09-03)** — OX 체결 테이프가 walking 한 가격대별로 최대
         6줄을 찍기 때문(§4 유저 체결 프린트 분해, 3행 × 6줄 = 최악 18행 / 실사용 평균 2~4줄). 이 단가가
         곧 반복 조건부·재체결 차단선의 환산율이므로 **줄 상한을 바꾸면 여기도 같이** 바꿀 것.
  ...
  - **⚠ 그래서 "0.5초·0.1초라도" 를 D1 없이 얻는 방법 = 클라 재생(`dripTrades`, 2026-08-19)**: 새 데이터의
    ...
    **6~26건(평균 ~12)** 이다. 그걸 통째로 top 에 꽂으면 "1초에 한 번 덜컥"으로 보이므로, 시간순으로 조금씩
