...
- **지정가(limitOpen)**: `pending_orders` 에 생성 시점 `limit_price` 기준 증거금을 즉시 잠금(조건부 UPDATE 동일 패턴). **실제 코인 38종**은 체결가를 재계산 없이 `limit_price` 그대로 사용(델타 정산 불필요). **OX/USDT 는 예외** — 실제 거래소처럼 marketable 지정가를 시장가로 즉시·가격개선 체결한다(§ OX/USDT "호가 역전 & 체결 안 됨" 버그 수정, `spot.ts fillOxPending`). `cancelLimit` 은 잠근 증거금을 그대로 환불.
...
- **⚠ 호가 역전(유저 매수 > 봇 매도) & "체결 안 됨" 버그와 그 근본 수정** — OX 는 사실상 **두 개의
  분리된 주문 풀**이 화면에서만 UNION 으로 합쳐 보였다: 봇 호가(`spot_orders`)는 봇끼리만 매칭
  (`matchBuy`/`matchSell`, `user_id != actor`)하고, 유저 지정가(`pending_orders`)는 그 유저 자신의
  `checkTriggers`(폴링)가 **스칼라 `ref_price`와만** 비교해 `limit_price` 그대로 체결했다. 그래서
  (1) 봇이 매 틱 `ref` 주변에 다시 까는 매도호가가 유저의 더 높은 매수 지정가를 **아예 안 보고**
  지나가 "내 매수(1.1111)보다 싼 매도(1.0919)가 계속 올라오는데 안 맞물리는" 이론상 불가능한
  호가 역전이 났고, (2) 체결이 **주문 낸 유저 개인의 `/api/state` 폴링에 전적으로 의존**해 탭이
  백그라운드로 가거나 폴링이 멈추면 시장이 그 가격을 뚫어도 영원히 안 채워졌으며, 채워져도
  `limit_price`(시장가보다 비싼 값)에 물렸다. **수정**: `spot.ts` 에 `fillOxPending()`(체결 가능
  여부·시장가 가격개선·증거금 정산·선점 기반 이중체결 방지)을 두고, 이를 **① `runMarketMaker()`
  가 매 폴링마다 전 유저의 체결 가능한 OX 지정가에 대해**(→ 주문 낸 유저의 접속 여부와 무관하게
  누구의 `/api/spot` 폴링·크론 5분마다 즉시 체결), **② `checkTriggers`(그 유저 폴링)**, **③
  `order.ts` limitOpen 제출 직후**(marketable 이면 즉시) 에서 공유 호출한다. 체결가는 트레이더에게
  항상 지정가 이상으로 유리하게(long=`min(limit,ref)`, short=`max(limit,ref)`) 개선하고, 생성 시
  잠근 증거금과의 차액을 환불/추가징수한다. 이래서 marketable 지정가는 실제 거래소처럼 호가창에
  남지 않고 즉시 체결·소멸 → 역전이 원천적으로 사라진다(실제 코인 38종은 별도 봇 시장이 없어 기존
  `limit_price` 체결 경로 그대로).
...
- [x] OX 지정가 = 실제 거래소식 매칭 통합(`fillOxPending`) — marketable 지정가는 시장가로 즉시·가격개선 체결(유저 개인 폴링 의존 제거, `runMarketMaker`가 전 유저 대상 처리), 유저 매수 > 봇 매도 호가 역전·"체결 안 됨" 근본 해결
