...
- **⚠⚠⚠ D1 예산 — 새 기능을 얹기 전에 여기부터 볼 것. 실제로 돈이 청구된 적이 있다.**
  ...
  **⚠⚠ 2026-08-14 부터 이 사이트는 Workers *Free* 플랜을 목표로 운영한다.** 그래서 판단 기준이 완전히
  ...
  - **⚠ 실사용 천장은 "동시 접속자"가 아니라 "하루 총 시청 시간(user-hour)"이다.** 폴링 3개가 각자
    ...
    | --- | --- | --- | --- |
    | 실제 코인 화면(state 폴링만) | 1,440 | 9.4만 | **53 user-hour** (읽기) |
    | OX + 1m 봉 | 8,640 | 23.4만 | **11.5 user-hour** (요청) |
    | OX + 15m 봉 | 8,640 | 32.6만 | **11.5 user-hour** (요청) |
    | OX + 30m 봉 | 8,640 | 44.8만 | **11 user-hour** (읽기·요청 동시) |
    측정치(2026-08-14, prod): `/api/state` 65행 · `/api/spot` 35행 · 캔들 폴링 1m 3행 / 15m 31행 /
    30m 61행(= limit 2 × 롤업 배수 + 진행 중 봉 1). cron 은 분당 ~27행(하루 3.9만).
    **더 필요해지면 폴링 3개를 한 엔드포인트로 합치는 게 가장 크다** — 신선도 손실 없이 요청이 1/2.4 로
    줄어 천장이 ~27 user-hour 가 된다(아직 안 함).
