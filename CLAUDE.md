...
│       ├── spot.ts         GET/POST /api/spot (OX/USDT 가상 코인 현물 — 유저 대 유저 지정가 주문매칭, 레버리지 없음)
...
└── src/                    ── 프론트 ──
    ...
    │   └── useTradingStore.ts  서버 상태 캐시(positions/orders/pendingOrders/refillsLeft) + init/login/logout/openMarket/closePosition/limitOpen/cancelLimit/setSlTp/refill
    └── components/
        ├── Login.tsx           이름+패스코드 로그인/가입
        ├── Header.tsx          심볼(38, 공용목록, 정렬 가능)/현재가/연결/잔고/리필버튼(평가자산<=0 일 때만 활성화, N/3)/OX버튼/랭킹버튼/설정버튼/로그아웃
        ├── SymbolSelect.tsx    심볼 드롭다운 — 심볼/가격/24h변동 표시, 컬럼 헤더 클릭으로 정렬(바이낸스 ticker/24hr, 열려있는 동안 5초 폴링)
        ├── OrderBook.tsx       호가창 — 상위 20호가(바이낸스 부분 호가 스트림, 1초 갱신), "모아보기" 배수 선택으로 인접 호가 묶어보기, 클릭 시 그 가격을 지정가 입력에 반영. Standard 모드 + 옵션(useChartStore.orderBook) 둘 다 켜져 있을 때만 표시
        ├── Settings.tsx        테마 3선택 + 거래모드(Easy/Standard) 2선택 + 폰트 크기 3선택 모달
        ├── Chart.tsx           Lightweight Charts: 타임프레임 그룹셀렉트(초봉 포함)·KST+9·OHLCV+인디케이터값 레전드(hover/터치)·다음봉 카운트다운·인디케이터(추가/삭제/기간편집)·매매 B/S/L 마커·포지션 평단선·SL/TP 수평선·차트 클릭→지정가 입력·테마 반응형 캔버스 재도색
        ├── OrderPanel.tsx      Easy=슬라이더로 비중만 정해 롱/숏 버튼(수량 텍스트입력·단위전환·호가창 전부 없음) / Standard=시장가+지정가 탭·SL·TP 입력·수량 텍스트입력+단위(코인/USDT) 전환 (레버리지는 공통, 체결가는 서버가 fetch)
        ├── PositionsPanel.tsx  탭: 포지션(청산가 표시·(Standard 전용) 부분청산 수량 입력·SL/TP 인라인 편집, Easy 는 전량청산 버튼만) / (Standard) 미체결 지정가 / 주문내역(전체 체결 이력, 강제청산 하이라이트)
        ├── SpotMarket.tsx      OX/USDT 가상 코인 현물 모달(Header "🪙 OX" 버튼) — 유저 간 지정가 매수/매도 주문매칭(레버리지 없음), 호가창·내 미체결·최근 체결, 3초 폴링
...
- **UI**: `Header.tsx` "🪙 OX" 버튼 → `SpotMarket.tsx` 모달(호가창·주문폼·내 미체결·최근 체결, 3초 폴링).
  기존 Chart/OrderPanel/PositionsPanel/SymbolSelect 는 건드리지 않음 — 완전히 독립된 화면.
...
- **⚠ 컬럼 마이그레이션(최초 1회, 수동)**: `CREATE TABLE IF NOT EXISTS` 는 이미 존재하는 prod 테이블에 새 컬럼을 추가해주지 않는다. 새 컬럼이 생길 때마다 `schema.sql` 맨 아래에 주석 처리된 `ALTER TABLE` 블록을 추가해두니, 배포 후 해당 줄들을 **한 번만** 직접 실행할 것:
  ...
  - `npx wrangler d1 execute ox64 --remote --command "ALTER TABLE users ADD COLUMN ox_balance REAL NOT NULL DEFAULT 100"` (OX 현물 거래) — `spot_orders`/`spot_trades` 는 신규 테이블이라 `--file=./schema.sql` 재적용만으로 자동 생성됨
