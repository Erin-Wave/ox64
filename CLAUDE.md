...
- **⚠ DB I/O 최소화(runMarketMaker 재작성, 2026-07-18)**: 예전엔 한 틱에 봇 호가 16개를 "개별 batch 로
  ...
  단 하나의 `DB.batch`(왕복 1회)** 로 처리하고, 재호가 게이트(3~8초)를 통과하지 못한 폴링은 **state read
...
- **캔들(차트)**: 외부 시세가 없으므로 `spot_trades`(봇끼리의 체결 기록)를 서버가 interval 단위로 JS
  ...
  거래량이 적어 SQL 윈도우함수 대신 **가장 최신** 거래 최대 5000건을 한 번에 읽어 그룹핑 — 모의투자
  규모에선 충분. ⚠ 반드시 최신 5000건이어야 한다(`ORDER BY created_at DESC LIMIT 5000` 후 다시 ASC 정렬):
  예전엔 `ASC LIMIT 5000`(가장 오래된 5000건)이라 총 거래가 5000건을 넘는 순간부터 새 거래가 창 밖으로
  밀려나 차트 마지막 봉이 그 시점에 멈춰버렸다("OX 차트 고장" 버그). 실시간 갱신은 WS 대신 `Chart.tsx` 가 3초마다 재요청(단, 표시 범위는 최초 로드 때만 설정 — 매
