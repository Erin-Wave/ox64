...
│   ├── _budget.ts          **D1 쓰기 예산 계량기 + 자동 쓰기 차단(서킷 브레이커)** — Cloudflare 엔 D1 지출 상한 기능이 없어서(예산 알림은 사후 통보) "포함분을 넘기면 멈춘다"를 코드로 만든 것. 스스로 반복해서 도는 경로(봇 틱·repeating 조건부)만 `usage_meter` 에 누적하고, 이번 달 누적이 `BLOCK_AT_ROWS`(포함분 5,000만의 90%)를 넘으면 그 경로들이 조용히 물러난다(수동 거래·트리거 sweep 은 계속 동작). 계량 문장은 **이미 도는 batch 에 얹어** 계량기 자체가 비용이 되지 않게 한다(§6)
...
└── src/                    ── 프론트 ──
    ...
    │   └── useTradingStore.ts  서버 상태 캐시(positions/orders/pendingOrders/refillsLeft) + init/login/logout/openMarket/closePosition/limitOpen/cancelLimit/setSlTp/refill(OXUSDT 도 이 경로 그대로 탐) + spotBook/spotTrades/spotRefresh(OX 호가창·체결내역 "표시용" 시장 데이터, 유저 개인 잔고 아님). **체결 목록은 `dripTrades` 가 0.5초 간격 2슬라이스로 흘려보낸다**(§6 — 봇 틱 하나가 3~12건을 한꺼번에 찍어서 목록이 1초에 한 번 덜컥 갱신되던 것을, 이미 받은 데이터를 시간순으로 내보내 초당 두 번 흐르게 한 것 — 요청·D1 증가 0). ⚠ 새 체결 식별은 **`createdAt`** 으로 한다 — 봇 테이프가 링 버퍼라 `id`(`t<시각>-<인덱스>`)가 폴링마다 바뀐다. 코인 전환 시 `spotClear` 가 남은 슬라이스 타이머를 반드시 지울 것
    └── components/
  ...
  **⚠⚠ 2026-08-14 부터 이 사이트는 Workers *Free* 플랜을 목표로 운영한다.** 그래서 판단 기준이 완전히
  ...
  - **⚠ 실사용 천장은 "동시 접속자"가 아니라 "하루 총 시청 시간(user-hour)"이다.** 폴링 3개가 각자
    ...
    **+16행**(트리거 평가 7 + loadState 9). cron 은 분당 ~27행(하루 3.9만).
    여기까지 오는 데 쓴 수단이 셋이고, 전부 "같은 걸 반복해서 다시 읽지 않는다"는 한 가지 원칙이다:
    ①**폴링 3개를 하나로**(요청 8,640→3,600/시간) ②**주문내역 증분**(`ordersSince` — 매번 같은 50행을
    다시 읽던 것을 0~1행으로) ③**체결내역 시간 범위**(항상 30행 → 평상시 1행).
    **다음 카드**: 폴링 주기(1초)를 늘리는 것 — 요청·읽기가 그대로 비례해 줄지만 체결 체감이 느려진다.
  ...
  - **⚠ 폴링 경로 전수 점검 결과(2026-08-01)** — 클라의 모든 주기 요청 중 **D1 에 쓰기를 만드는 건 둘뿐**이고
    ...
    | --- | --- | --- | --- |
    | `useSpotPoll` → `/api/state?tick=` | 1s (탭 숨기면 정지) | **봇 커밋 1행**(게이트 0.45~0.95s) + 체결 시 20행 — 계량·차단 대상 | 3,600/시 |
  ...
  - **⚠ 그래서 "0.5초라도" 를 D1 없이 얻는 방법 = 클라 슬라이스 재생(`dripTrades`, 2026-08-19)**: 새 데이터의
    **도착** 주기는 폴링(=요청)에 묶여 있어 못 줄이지만, 한 폴링에 오는 건 봇 틱 하나가 찍은 체결
    **3~12건**이다. 그걸 통째로 top 에 꽂으면 "1초에 한 번 덜컥"으로 보이므로, 시간순으로 절반씩 0.5초
    간격으로 내보낸다 → 목록이 초당 두 번 흐른다. **이미 받은 데이터를 순서대로 꺼내는 것이라 요청·읽기·
    쓰기가 하나도 안 늘고**, 대가는 그 묶음의 가장 새 체결이 최대 0.5초 늦게 보이는 것뿐이다(헤더 현재가·
    차트는 그대로 즉시 갱신 → 목록 맨 위가 현재가를 0.5초 뒤따라간다 = 실제 거래소 테이프와 같은 모양).
...
- [x] **체결 목록 0.5초 흐름(클라 슬라이스 재생) + 체결 수량 방향색** — 폴링 주기(1초)는 요청·쓰기 한도 때문에 못 줄이지만, 한 폴링에 오는 체결 3~12건을 0.5초 간격 두 슬라이스로 내보내 목록이 초당 두 번 흐르게 했다(`dripTrades` — **요청·D1 읽기·쓰기 증가 0**, 대가는 가장 새 체결이 최대 0.5초 늦게 보이는 것). 체결 행의 수량도 가격과 같은 테이커 방향색으로. 30회 폴링 시뮬레이션으로 정렬 유지·이미 본 체결 보존·누락 0 검증
