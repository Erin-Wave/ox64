...
│       ├── order.ts        POST /api/order  (open/close/limitClose/limitOpen/cancelLimit/setSlTp — 서버가 체결가 fetch·손익 계산·D1 원자 갱신. close 는 OX 면 봇 호가창 walking 청산, limitClose 는 지정가 청산=reduce-only)
