...
├── scripts/                운영 스크립트 — d1-budget.mjs(D1 쓰기 예산 점검, §6) · sim-bot.ts(봇 심리 모델 장기 시뮬레이션 + **체결 미세구조 검증**, `npm run sim:bot` — 심리 파라미터를 바꿨으면 반드시 돌릴 것. 기본 7일=정확히 한 주여야 세션 활성도 평균이 1 로 나온다. 편향은 종가가 아니라 **로그드리프트/틱**(산술평균−분산/2, |값| 2e-6 이하)으로 본다)
...
- **⚠ 봇 매매 심리 모델(`nextMarketState`, 2026-07-20)**: 예전 기준가는
  ...
  **`spot_bot_state` 행에 얹혀 틱 사이에 지속**되므로 추가 DB 왕복이 없다(어차피 읽고 쓰던 행 — 컬럼이
  ...
  - **⚠⚠ 체결 미세구조 — "가격은 누가 호가를 때렸나에서 나온다"(`simulateTick`, 2026-09-07 재설계)**:
    ...
    - **⚠ 틱 내부 체결 시각은 그 틱의 10ms 창 안에 고르게 편다**(`TICK_PRINT_SPAN_MS`) — 예전 `now + i` 는
      건수가 11 을 넘으면 다음 틱의 시각(`runBotTicks` 의 틱 간 최소 간격 10ms)을 넘어서서 **테이프 정렬이
      틱 경계에서 뒤섞였다**(prod 실측으로 시각 역행 0건 확인). 이 창을 10ms 보다 크게 잡으면 안 된다.
