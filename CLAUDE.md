...
- [x] **체결창 필터 · 강세/약세 인디케이터 · 고정 높이 + 레버리지 250배 + 큰 숫자 축약 확대(2026-09-03)** — ①체결 목록을 **거래대금(USDT) 또는 수량** 기준 "이상/이하"로 거르는 설정(전 심볼 공통, 표시 전용, 필터 뱃지 클릭 시 즉시 해제). 필터가 세면 목록이 비어 보이므로 클라 테이프 버퍼를 50→400 으로 키우고 가상 코인은 교체가 아니라 **새 것만 얹도록**(`mergeTrades`) 바꿨다(요청·D1 증가 0). ②체결 가격 옆 **강세▲/약세▼**(직전 체결 대비, tick rule) — 색(테이커 방향)과 다른 정보라 설정으로 끌 수 있다. ③호가·체결 목록 높이를 **고정**(maxHeight→height) — 체결이 한 건씩 흘러들어올 때마다 패널이 오르내려 아래가 밀리던 문제. ④레버리지 상한 200→**250**(서버 4곳 + 슬라이더). ⑤**가격도 축약**(`fmtPriceShort`) — 청산가가 1e20 을 넘어 표를 밀어내던 버그(전체값은 title). ⑥포지션 **청산 수량 슬라이더**. 로컬 D1 검증: 레버리지 250 수용·251 거부, 포지션/체결 정상
