...
├── schema.sql              D1 스키마(users[+refill_count/refill_date/ox_balance]/positions/orders/pending_orders[+reduce_only=지정가 청산]/spot_orders/spot_trades/spot_candles[OX 영속 캔들]/spot_bot_state[+drift/vol/sentiment/anchor/regime/regime_ticks=봇 심리상태]) — wrangler d1 execute 또는 D1 Console 로 적용
...
- **⚠ 거래 수수료 + VIP 등급(2026-07-20)**: 모든 체결에 `수수료 = 명목금액(체결가×수량) × VIP 요율` 이 붙는다.
  ...
  - **수익 원장 = `fee_ledger`**(체결 1건당 1행: user/symbol/kind/notional/rate/fee/created_at). 플랫폼 수수료 수익의 진실원본이며 `SUM(fee)` 로 집계한다. `users.total_fees` 는 유저 표시용 캐시.
  ...
  - UI: `VipBadge.tsx`(헤더 이름 옆·모바일 더보기·랭킹 각 행, title 에 요율/다음 등급까지 남은 금액), `OrderPanel` 정보란에 예상 수수료 + 현재 등급/요율.
- **아직 없음**: 펀딩비.
...
- **⚠ 봇도 거래 수수료를 낸다(`botFeeStmts`)**: 합성 체결(봇끼리)이든 유저 상대 체결(maker 로 잡힌 물량)이든 봇도 요율을 적용받아 `fee_ledger` 에 기록된다. 봇 잔고는 어디서도 안 읽히는 무한 풀이라 잔고 차감은 의미가 없지만, **시장 물량의 대부분이 봇에서 나오는데 봇만 면제하면 수수료 원장이 실제 거래량과 동떨어진다**. 요율은 유저와 똑같이 누적 거래대금에서 파생(`vipOf`)하므로 봇도 거래가 쌓이면 등급이 오른다(특혜 없음). 원장은 `kind='bot'` + `user_id='bot-mm-*'` 라 유저 수수료와 분리 집계할 수 있다.
