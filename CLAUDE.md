...
├── cron/                   ── 강제청산 전용 Cron Worker (메인 Pages 프로젝트와 별도 배포) ──
│   ├── wrangler.toml       name="ox64-liquidation-cron", 같은 D1(ox64) 바인딩, [triggers] crons=["0 * * * *"](매시)
│   └── index.ts            scheduled() 가 functions/_trading.ts sweepForcedLiquidations() 호출. fetch() 는 CRON_SECRET 헤더로 보호된 수동 트리거(테스트/즉시 재실행용)
...
### 강제청산 Cron Worker (`cron/`) — 메인 Pages 배포와 별개, **최초 1회 수동 배포 필요**
- Cloudflare Pages 프로젝트는 Cron Trigger 를 지원하지 않는다(Durable Objects 도 Pages 안에서 새로 정의 불가 — 둘 다 별도 Worker 배포가 필요). 그래서 `cron/` 를 **완전히 별개의 Workers 프로젝트**로 배포해야 한다(Git 연동 Pages 배포로는 자동 적용되지 않음).
- 최초 배포:
  ```bash
  cd cron
  npx wrangler deploy                                          # name="ox64-liquidation-cron" 로 신규 Worker 생성
  npx wrangler secret put CRON_SECRET --name ox64-liquidation-cron   # 수동 트리거 fetch() 보호용, 랜덤 값 아무거나
  ```
- 이후 코드/스케줄 변경 시 `cd cron && npx wrangler deploy` 로 재배포(Pages 처럼 Git 연동 자동배포 아님 — 수동).
- 수동 재실행/점검: `curl -X POST https://ox64-liquidation-cron.<account>.workers.dev/ -H "x-cron-secret: <값>"` → `{"checked":N,"liquidated":M}`.
- 주기는 `cron/wrangler.toml` 의 `[triggers] crons`(현재 매시 정각) — 로컬 검증은 `cd cron && npx wrangler dev` 뒤 `curl http://127.0.0.1:8787/cdn-cgi/handler/scheduled` (스케줄은 로컬에서 자동 발화 안 됨, 수동 트리거만).
...
- [x] 강제청산을 접속 여부와 무관하게 매시 자동 실행(cron/ 별도 Worker, sweepForcedLiquidations)
- [ ] 수수료·펀딩비 반영
