...
- **입력 검증**: 심볼 형식(USDT 페어), side∈long/short, `size>0 && size<=1e15`, leverage 1~200. ⚠ 수량 상한은 예전 `1,000,000` 이었는데 PEPE 등 **싼 코인은 정상적으로 수십억 개**를 거래해서 "수량 오류"가 나던 버그가 있었다 → `1e15` 로 완화(실제 한도는 `margin<=balance` 조건이 잡아주고, 이 캡은 부동소수 폭주 방지용 안전장치).
...
- **⚠ 무한(반복) 조건부(`repeating`, 2026-07-28)**: "1.5 이하로 떨어질 때마다 시장가 123개 매수"처럼 **체결돼도 주문이 사라지지 않고** 계속 일하는 모드. `conditional_orders.repeating=1` 이면 `size` 는 남은 목표가 아니라 **1회 실행 수량(차감 안 함)** 이고, 체결 후 행을 지우는 대신 `fill_count+1` · `armed=0`(재무장 대기)으로 되돌린다.
  - **⚠ 재무장(`armed`/`rearm_price`)이 이 기능의 핵심 안전장치**: 조건만 유지하면 "1.5 이하"가 참인 동안 **폴링마다(2.5초에 한 번) 계속 사들여** 순식간에 파산한다 — **"떨어질 때마다" ≠ "떨어져 있는 동안 계속"**. 그래서 한 번 실행되면 무장을 풀고, 가격이 **트리거 반대편**(`rearm_price`, 미설정이면 `trigger_price`)으로 돌아왔을 때만 `armed=1` 로 복구한다(below 면 `mark >= rearm`, above 면 `mark <= rearm`). `settleConditionalOrder` 는 재무장 대기 중이면 **재무장 판정만 하고 즉시 return**(그 폴링에선 절대 체결 안 함)한다.
  - `rearm_price` 는 방향 검증을 받는다 — below 는 `rearm >= trigger`, above 는 `rearm <= trigger`(반대편이 아니면 재무장이 성립하지 않는다). 히스테리시스를 크게 주면 그만큼 실행 빈도가 낮아진다.
  - `max_fills`(1~100,000, NULL=무제한)에 도달하면 그 체결 batch 안에서 주문을 **삭제**한다. `fill_count` 는 표시용.
  - 체결 후 행 처리는 OX/실제코인 양쪽이 `conditionalAfterFillStmt()` 하나를 공유한다(1회성=잔량 차감/삭제, 무한=횟수+1·재무장 대기·상한 도달 시 삭제). OX 는 `filled > EPS` 일 때만 부르고(유동성 부족으로 0 체결이면 무장 상태 그대로 유지 → 다음 폴링 재시도), 실제 코인은 잔고 차감 가드를 통과한 뒤 같은 batch 에 얹는다.
  - **⚠ 신규 컬럼은 마이그레이션 전 DB 에서 `undefined` 로 온다** — 읽는 쪽(`loadState`/`settleConditionalOrder`)은 전부 `?? 0`/`?? 1` 로 방어하지만 `conditionalOpen` 의 INSERT 는 컬럼이 없으면 실패하므로 **코드 배포 전에 ALTER 를 먼저 적용**할 것(§5).
  - UI: `OrderPanel` 조건부 탭의 "무한 반복" 체크박스(켜면 재무장 가격·최대 실행 횟수 입력 + 동작 설명), `PositionsPanel` 조건부 탭의 "반복" 컬럼(무한 ∞ / N회 중 M회 · 대기 중 / 재무장 대기), `Chart` 주문선 라벨 `조건부∞`(재무장 대기면 재무장 가격에 흐린 점선을 하나 더 그려 "왜 지금은 안 걸리는지"를 보여준다).
...
- **⚠ 거래 수수료 + VIP 등급(2026-07-20)**: 모든 체결에 `수수료 = 명목금액(체결가×수량) × VIP 요율` 이 붙는다.
  - **등급 = 누적 거래대금(`users.total_volume`)** 으로 결정. 증거금이 아니라 **명목금액(레버리지 포함)** 이라 고배율일수록 빨리 오른다. 진입·청산 각각 그 체결의 명목금액만큼 누적. VIP0~12, **1단계당 100배**(2026-07-24 에 VIP4 가 너무 쉬워 5~12 프레스티지 등급을 같은 100배 간격으로 이어붙임).
    | 등급 | 누적 거래대금(USDT) | 요율 |
    | --- | --- | --- |
    | VIP0 | ~100만 | 0.03% |
    | VIP1 | 100만~1억 | 0.02% |
    ...
    | VIP6 | 1경~100경 | 0.0002% |
    | VIP7 | 100경~1해 | 0.0001% |
    | VIP8 | 1해~100해 | 0.00005% |
    | VIP9 | 100해~1자 | 0.00002% |
    | VIP10 | 1자~100자 | 0.00001% |
    | VIP11 | 100자~1양 | 0.000005% |
    | VIP12 | 1양~ | 0.000002% |
    - ⚠ VIP5+ 요율은 0.001% 미만이라 표시를 **`format.ts fmtFeeRate`(소수 6자리)** 로 통일했다(예전 `toFixed(3)` 은 0.001% 미만을 "0" 으로 뭉갰다). `VipBadge` 색 스케일도 13단계로 확장(VIP5~12 골드→그라디언트). `fmtKor` 에 경(1e16)/해(1e20)/자(1e24)/양(1e28) 단위 추가(1e28 은 double 정밀도를 넘지만 등급 판정은 크기 비교라 무관).
...
- **⚠ 동기화 = Durable Objects/WebSocket 이 아니라 D1 + 짧은 폴링**: 진짜 실시간(수십 ms) 응답을
  ...
  없이(같은 Pages 배포 안에서) 구현했다. 모든 액션(POST)은 자기 응답으로 즉시 상태를 갱신하므로
...
- [x] **무한(반복) 조건부 주문** — 체결돼도 주문이 남아 "트리거될 때마다" 반복 실행(`repeating`/`armed`/`rearm_price`/`fill_count`/`max_fills`). ⚠ 재무장(트리거 반대편으로 돌아와야 다시 무장)이 없으면 조건이 참인 동안 폴링마다 사들여 파산하므로 필수. 로컬 D1 검증: 즉시 1회 실행 후 5회 폴링 동안 재실행 0건, 재무장 후 정확히 1회 재실행, max_fills 도달 시 자동 삭제, 1회성 주문 동작 무회귀(OX/실제코인 양쪽)
