...
└── src/                    ── 프론트 ──
    ...
    └── components/
        ...
        ├── Header.tsx          심볼(38, 공용목록, 정렬 가능)/현재가/연결/잔고/리필버튼(평가자산<=0 일 때만 활성화, N/3)/랭킹버튼/설정버튼/로그아웃
        ├── SymbolSelect.tsx    심볼 드롭다운 — 심볼/가격/24h변동 표시, 컬럼 헤더 클릭으로 정렬(바이낸스 ticker/24hr, 열려있는 동안 5초 폴링)
        ├── OrderBook.tsx       호가창 — 상위 20호가(바이낸스 부분 호가 스트림, 1초 갱신), "모아보기" 배수 선택으로 인접 호가 묶어보기, 클릭 시 그 가격을 지정가 입력에 반영
...
- [x] 부분 청산
- [x] 호가창(바이낸스 부분 호가 스트림) + 모아보기(가격 그룹핑) + 클릭 시 지정가 반영
- [ ] 수수료·펀딩비 반영
