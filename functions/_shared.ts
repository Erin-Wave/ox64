...
// 1e60 이면 BTC 최고가(~1e5)를 곱한 명목금액도 1e65 라 double(1.8e308) 에서 한참 멀고, 그만큼 사려면
// 잔고가 1e56 이어야 해서 정상 거래로는 절대 닿지 않는다.
export const MAX_ORDER_SIZE = 1e60;

/** ⚠ 상한을 넘는 수량은 **거부하지 말고 자른다**(clamp). 예전엔 `size > MAX_ORDER_SIZE` 를 그대로
 * "수량 오류"로 반환했는데, 유저가 입력칸에 자릿수를 길게 넣으면(888…888 39자리) 그게 곧 에러라서
 * "주문이 아예 안 되는" 것처럼 보였다 — 상한은 계산 안전장치일 뿐이므로 사람이 읽을 이유가 없다.
 * 자른 뒤에는 어차피 증거금 가드가 판정한다: 실제 코인은 "증거금이 부족합니다"(정확한 사유),
 * OX/EW 는 감당 가능한 수량으로 클램프해 부분 체결(§4 시장가 매칭). Infinity(자릿수 300+ 나
 * "1e999")도 `> MAX` 가 참이라 여기서 흡수되고, NaN 은 비교가 거짓이라 그대로 통과해
 * order.ts `badSize` 가 잡는다. */
export const clampOrderSize = (size: number) => (size > MAX_ORDER_SIZE ? MAX_ORDER_SIZE : size);
